...
export async function onRequest(ctx:EventContext<Env>):Promise<Response>{
  ...
  try{
    ...
    return Response.json({success:true,answer:ans+disclaimer,sources:srcs,query,timestamp:new Date().toISOString()},{headers:CORS})
  }catch(e){
    console.error('Search error:',e)
    return Response.json({success:false,message:'검색 처리 중 오류가 발생했습니다',error:String(e)},{status:500,headers:CORS})
  }
}
